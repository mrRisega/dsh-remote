...
## [0.6.9-beta.1] - 2026-09-19

> **修「更新/补装永久卡死」**（用户侧 0.6.8 诊断报告的核心问题）+ 安装/设置入口文案纠正 + 面板邀请区与更新状态 UI 重做。
>
> 报告的核心结论一句话：**三套既有的"卡死治理"判据全选错了** —— 它们问的是"标记还在不在 /
> 进程还活不活"，没有一套在问"**它还在不在往前推进**"。而中国网络访问 `registry.npmjs.org` 的
> **典型**失败形态恰恰是**挂起**（TCP 连得上、传输不动）：进程活着、标记是新的、却永远不产生
> 任何事件（没有 exit、没有新日志）。于是镜像回退（只认退出码）永不触发、标记永不清除、
> 用户永久停在「更新中 / 正在安装」—— 而装机数就是不涨。

### Fixed

- **更新卡死（P1，致命）**：给更新子进程加**无输出看门狗** —— 判据从"标记年龄"换成
  "日志文件多久没有新增字节"（进度信号是现成的，两条路径本来就把子进程输出写进日志；
  用 mtime 判定在**宿主重启后依然有效**，内存里的看门狗做不到）。超过阈值（默认 120s，
  `DSH_UPDATE_IDLE_MS` 可覆盖）即判定卡住 → 结束该子进程 → **走与 `exit≠0` 完全相同的回退路径**
  （自动换 npmmirror 重试）。"永久卡死"因此变成"最多两分钟后自动换源成功或如实报错"。
- **补装卡死（同一根因，直接影响装机数）**：`ensureRuntime`（市场装完插件后的运行环境补装）
  同样装上该看门狗 —— 用户说的"面板注册完成了但设备没上报"，很大一部分就是补装的 npx
  在这条链路上挂起、而旧实现只在"pid 死 或 标记超 10 分钟"时才清理。卡住即结束并按
  `runtime_install_timeout` 归因（该码已在上报白名单内），下一轮自愈重试。
- **孤儿进程累积（P2）**：挂死的子进程**从不会被结束**（`child.kill()` 全文无匹配，标记里的 pid
  只被用来"判活"、从未被用来"结束"）→ 用户每次重试都堆一个 node 进程。现在宿主启动清理、
  轮询自愈、取消三处都会**先结束再清标记**（判据同样是"日志多久没变化"，不看进程存活）。
- **卡住对用户与统计都不可见（F2）**：`/self/update-log` 新增 `channel / elapsedMs / idleMs /
  idleThresholdMs / stalled`；卡住会记 `update_stalled` 并**上报**（新增归因码，服务端白名单已同步）。
  此前失败只在 `exit≠0` 时记录，所以遥测里永远看不到"卡死"这一类，规模被持续低估。
- **更新没有自愈引擎（F5）**：补装之所以能自愈，靠的是"每次轮询都再试一次"；而更新路径只有
  一次性 fire-and-forget（全文一个调用点），一旦第一次卡住就再没有任何后续。现在面板的
  2~3s 轮询（`ensureConnection`）会顺手治一次卡住的更新 —— 复用已有轮询，不新增定时器。
- **通道方向相反（P3）**：原来无条件拉 `latest`，预发版装机点「一键更新」等于**降级或什么都不做**，
  而面板既不显示通道也不沿用通道。现在：显式 `DSH_UPDATE_TAG` 优先，否则**沿用装机通道**，
  并且在「当前通道」与 `latest` 之间**取更高者**（开发期 beta 领先、正式版发布后 latest 反超，
  取更高者对两种情况都成立）；`/self` 与 `/self/update-log` 都下发 `channel` 供面板显示。
- **取消更新（新增）**：`POST /self/update/cancel` —— 结束挂死的更新进程 + 清标记。
  面板在"疑似卡住"时给按钮；取消**不算失败遥测**（只在本地留一条可查询记录）。
- **潜伏缺陷**：`readMarkerInfo` 里 `raw` 声明在 try 块内却在块外使用 —— 对**旧格式（纯时间戳数字）
  标记**会抛 `ReferenceError`，而所有调用点都用 try/catch 吞掉，表现是**旧格式标记永远不会被清理**。
  已修（并由新用例覆盖）。
- **安装引导指向了不存在的菜单**：安装器有 **13 处**让用户去「设置 → 远程控制」，而面板早已改名
  「远程访问」→ 照做的用户**根本找不到入口**。已全部改为「远程访问」（README 同步）。
- **市场搜索词**：README 的安装引导原写"搜索 `dsh-remote`"，正确关键词是 **`dsh-remote-web`**。

### Added

- **`POST /dsh-remote/self/update/cancel`**：取消正在进行的更新。
- **`test/update-stall.test.mjs`（8 个用例）**：用**真的会卡住的子进程**（假 npx 只 sleep、零输出）
  把链路压到亚秒级验证：判定卡住 → 结束进程 → 换源重试 → 如实记 `update_stalled` → 标记清掉；
  并含反例"有持续输出时绝不被误杀"、孤儿回收、轮询驱动自愈、取消后不再重试、通道选择、
  旧格式标记清理、补装看门狗。
- 遥测文档 `docs/telemetry.md` 补 `update_stalled` 归因码说明（失败码白名单有测试交叉校验）。

### Changed

**插件面板（设置 → 远程访问）整体重做**

- **邀请卡重排**：奖励大字（「邀请 **3 位**好友 → 你得 **15 天 PRO 会员**」，数值取自
  `public-config.invite_rule`，不编造）→ 进度框（已邀请/已到账/进度条/再邀几位）→
  邀请码与邀请链接**一键复制**（成功变绿、失败红字贴在同一行下方）→ 活动规则**默认折叠**
  → 邀请记录列表（手机号打码、状态、时间、已到账奖励）。未登录/无码/无记录三种空状态都给了
  明确操作入口。顺手修掉一个既有缺陷：`invite-records` 请求失败以前被静默当成"没有记录"。
- **新增两个邀请入口**（对应"给他一个出发点"）：①**免费额度将尽/已被限速**时
  （`quota.limit_enabled && !isMember && (percent≥80 || !max_mbps)`）；
  ②**连接明显偏慢**时（`attempts≥3` / 阶段文案含限速类词 / 本机观察非 online 持续 ≥90s）。
  两处都只是一句灰字 + 文字链，不弹窗、不抢主 CTA；判断不了就不显示（只用现有接口字段）。
- **更新状态可见**：显示**当前通道**（稳定版/预览版/指定版本）与**已用时 / 距上次输出**；
  `stalled=true` 时给黄色告警「疑似卡住（已 N 秒无输出），正在自动换源重试…」，
  超过阈值出现「取消并重试」（调 `POST /self/update/cancel`，404 时永久隐藏该按钮并给降级说明）；
  新增 `update_stalled` / `user_cancelled` 等人话翻译。所有新字段**特性检测**，旧 node 半上自动降级不报错。
- **文案统一**：`bridge` → **「后台服务」**（并在卡片首行加一句人话解释内部名 bridge）；
  本机＝「这台电脑」、会话/手机＝「设备」、订阅/档位＝「套餐」；栏目副标题补上产品定位一行。
- **可用性硬规则**：全部触控目标 ≥44px（行内链与侧栏入口用 `::after` 扩热区、不破坏与官方按钮的对齐）；
  恢复并强化键盘焦点环（删掉了 `outline:none`）；把面板里 8 处对比度仅 2.85:1 的灰字
  （`#8c959f`）换成 ≥4.5:1；`prefers-reduced-motion` 全量降级；hover/active 反馈补齐；
  复制失败、更新失败等错误提示**就近**显示；窄屏收紧布局。
...
- **悬浮按钮默认挪到右上角，并且可以拖到任何位置**。原来固定在右下角，与输入区的发送按钮重叠，
  手机上很容易误触；现在默认贴右上角（距边 10px），长按拖动即可移动，位置按「最近的角 + 距边偏移」
  记住 —— 旋转屏幕、换窗口宽度都会跟着重算并夹在屏内，不会跑到看不见的地方。
- 菜单跟随按钮：按钮在上半屏就挂在它下方、在下半屏挂上方；靠左/靠右分别对齐，都不超出视口。
- 触屏拖动用指针事件 + 指针捕获（手指拖出按钮范围也不断线），6px 阈值区分「轻点」与「拖动」。

### Fixed

- **修掉一个自引入的回归**：第一版为防止拖动时页面滚动用了 `preventDefault()`，
  实测它会连带掐掉随后的 click 事件 —— 表现是「按钮点一下没反应、菜单打不开」。
  现在改由 CSS `touch-action: none` 防滚动，轻点在 `pointerup` 里直接开合菜单。

### 开发者相关

- `scripts/release-tarballs.sh` 生成 Release 说明用的 `sed` 是 GNU 专用写法，
  在 macOS（BSD sed）上会报错并中止 —— 发 0.6.7 时实测踩到（Release 没建成、市场资产没上传）。
  已改为纯 awk 实现。
