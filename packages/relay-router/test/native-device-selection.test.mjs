...
test("不含升级/付费页:推广页、价格对比、支付弹层都不在本版本", () => {
  // 本仓库只提供客户端(登录 + 设备选择 + 进入镜像),升级/付费/邀请属托管服务能力,不在开源侧。
  assert.doesNotMatch(APP, /renderPromo|promo-tab-monthly|promo-compare|promo-billing-tabs/);
  assert.doesNotMatch(APP, /qr-modal|payask-done|payask-cancel|retain-modal/);
  assert.doesNotMatch(APP, /buy_click|upgrade-intent/);
  // /app/promo 路径保留兼容:带查询参数回应用根路径,一次性登录与邀请链接照旧可用
  assert.match(APP, /endsWith\("\/promo"\)/);
  assert.match(APP, /window\.location\.replace\("\/app\/" \+ \(window\.location\.search/);
});
...
test("/app/promo 兼容跳转:带查询参数回应用根路径,一次性登录链接不失效", () => {
  assert.match(APP, /endsWith\("\/promo"\)/);
  // 原样带上 search,因此 ?auth=KEY / ?invite=CODE 仍会被根路径的既有分支消费
  assert.match(APP, /window\.location\.replace\("\/app\/" \+ \(window\.location\.search/);
  // 一次性登录端点与失败文案仍在(由根路径 auth 分支使用)
...
// 首装引导:手机端空设备态不再让用户"等不到就算了"。
// 原因是引导只给了「装 Node.js + 终端跑 npx」这一条技术路径,技术门槛把不少人挡在门外。
