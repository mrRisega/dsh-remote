...
//
// 背景(生产实测):企业端 devices.online 列历史上只置 1、无清 0 路径 —— 15/15 台设备恒显示"在线",
// 而 last_seen_at 只在 bridge 重新登记时更新,长时间运行的 bridge 会被误判离线。router 是唯一
// 真正知道 bridge 是否在线的进程,因此需要在 ①注册 ②断开 ③每 60s 心跳 三个时机上报。
...
test("三个上报时机:注册成功→true;断开(且确实从在线表摘除)→false;每 60s 心跳补 true", () => {
  // ① 注册成功(bridge 注册日志下一行即上报)
  assert.match(SRC, /\[router\] bridge 注册:[\s\S]{0,400}?reportPresence\(deviceId, dev\.userId, true\)/);
