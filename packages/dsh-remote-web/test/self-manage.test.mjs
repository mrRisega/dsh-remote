...
test("★ 用户反馈锁：已是最新版时，修复入口必须**看得出是修复入口**（不能只叫「重新检查」）", () => {
  // 用户原话（fb_4cc2c9df749c）：「我的端口不是 3080，**没找到一键更新的按钮**」。
  // 原因：npm 上已是最新版时，主按钮文案是「重新检查 / 修复」—— 他按字面找"一键更新"，
  // 找不到就以为面板没有修复入口。现在文案直接自称修复，并把"该点什么"写在旁边。
  const source = readFileSync(new URL("../lib/client.js", import.meta.url), "utf8");
  assert.match(source, /一键修复 \/ 重装运行环境/, "主按钮必须直接自称修复入口");
  // ⚠️ 只查**字符串字面量**：这条规则写在注释里（说明历史文案）是允许的，出现在按钮上不行。
  assert.ok(!/"重新检查 \/ 修复"/.test(source), "旧的模糊按钮文案不得再出现（用户已经因此找不到入口）");
  assert.match(source, /已是最新版本 v/, "要如实告诉用户「已是最新」，并说明该怎么做");
  assert.match(source, /会重新补装并拉起后台服务/, "要说清这一步到底做什么");
});
